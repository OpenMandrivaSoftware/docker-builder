...
RUN dnf5 --refresh --assumeyes --no-docs --setopt=install_weak_deps=False upgrade \
 ...
 && dnf5 --assumeyes --setopt=install_weak_deps=False --no-docs install mock git coreutils curl sudo builder-c 'dnf-command(builddep)' 'dnf5-command(builddep)' procps-ng tar locales-en \
...
RUN if [ $RARCH = "x86_64" ]; then dnf5 --assumeyes install qemu-static-aarch64 qemu-static-arm qemu-static-riscv64; fi
