...
RUN dnf5 --no-gpgcheck --refresh --assumeyes --no-docs --setopt=install_weak_deps=False upgrade \
 ...
 && dnf5 --no-gpgcheck --assumeyes --setopt=install_weak_deps=False --no-docs install mock git coreutils curl sudo builder-c 'dnf-command(builddep)' 'dnf5-command(builddep)' procps-ng tar locales-en \
