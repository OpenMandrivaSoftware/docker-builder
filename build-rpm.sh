...
if [ ! -z "$HTTP_PROXY" ] || [ ! -z "$http_proxy" ]; then
        ...
        cp $MOCK_CONF_FOLDER/$MOCK_CONFIG.cfg /tmp/$MOCK_CONFIG.cfg
        sed s/\\[main\\]/\[main\]\\\nproxy=$TEMP_PROXY/g /tmp/$MOCK_CONFIG.cfg > $MOCK_CONF_FOLDER/$MOCK_CONFIG.cfg
fi
...
if [ ! -z "$SOURCE_RPM" ]; then
        ...
        echo "========================================================================"
        $MOCK_BIN -v --configdir=$MOCK_CONF_FOLDER -r $MOCK_CONFIG --rebuild $MOUNT_POINT/$SOURCE_RPM --resultdir=$OUTPUT_FOLDER
        ...
        echo "========================================================================"
        $MOCK_BIN -v --configdir=$MOCK_CONF_FOLDER -r $MOCK_CONFIG --buildsrpm --spec=$MOUNT_POINT/$SPEC_FILE --sources=$MOUNT_POINT/$SOURCES --resultdir=$OUTPUT_FOLDER
        $MOCK_BIN -v --configdir=$MOCK_CONF_FOLDER -r $MOCK_CONFIG --rebuild $(find $OUTPUT_FOLDER -type f -name "*.src.rpm") --resultdir=$OUTPUT_FOLDER
fi
